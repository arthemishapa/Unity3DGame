...
namespace MalbersAnimations.Controller
{
    ...
    public partial class MAnimal
    {
        void Awake()
        {
            ...
            foreach (var mode in modes)  mode.AwakeMode(this); //Awake all modes
        ...
        protected virtual void AdditionalTurn(float time)
        {
            ...
            if (MovementDetected)
            {
                ...
                if (MoveWithDirection)
                {
                   ...
                   // transform.RotateAround(transform.position, Up, DeltaAngle * time * (TurnMultiplier + 1));
                }
                else
                {
        ...
        protected virtual void AdditionalSpeed(float time)
        {
            InertiaPositionSpeed = (CurrentSpeedModifier.lerpPosition > 0)
                ? Vector3.Lerp(InertiaPositionSpeed, TargetSpeed, time * CurrentSpeedModifier.lerpPosition) : TargetSpeed;
        ...
        protected void MovementSystem(float DeltaTime)
        {
            ...
            else
            {
                ...
                if (Mathf.Abs(HorizontalSmooth) < 0.001f) HorizontalSmooth = 0;
                if (Mathf.Abs(UpDownSmooth) < 0.001f) UpDownSmooth = 0;
        ...
        void OnAnimatorMove()
        {
            ...
            if (InputMode != null)  InputMode.TryActivate(); //FOR MODES  Still need to work this better
        ...
        private void GravityLogic()
        {
            if (UseGravity)
            {
                ...
                if (GoingDown)
                {
                    ...
                    if (gravityMagn < 0.01f * ScaleFactor && InertiaMagn > 1f) //This means the Animal isnot falling properly
                    {
                        GravityStoredAceleration = 0;
