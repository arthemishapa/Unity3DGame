...
namespace MalbersAnimations.Controller
{
    public class MAnimalAIControl : MonoBehaviour , IAIControl
    {
        ...
        public virtual void Stop()
        {
            ...
            if (IsOnMode) animal.Mode_Interrupt();
        ...
        protected virtual void OnGrounded(bool grounded)
        {
            ...
            // Debuging(name + ": Grounded => "+ grounded);
            ...
            if (IsGrounded)
            {
                ...
                {
                    //Debuging("Setting Destination Agent");
                    ResumeAgent();
                    Agent.SetDestination(TargetPosition);                       //If there's a position to go to set it as destination
                }
            }
            else
            {   //Needs to pause the AGENT since the animal is no longer on the ground and NavMesh
        ...
        protected void Debuging(string Log, GameObject obj)
        {
            if (debug) Debug.Log(Log, obj);
